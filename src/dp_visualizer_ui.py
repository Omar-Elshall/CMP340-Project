#!/usr/bin/env python3
"""
Web-based DP Table Visualizer for Longest Common Subsequence
"""
...
class WebVisualizer(http.server.SimpleHTTPRequestHandler):
    """HTTP handler for DP visualization"""
...
def start_server(port=8000):
    ...
    try:
        # Try to use the requested port, fall back to a random one if busy
        httpd = socketserver.TCPServer(("", port), handler)
    except OSError:
        # Port is busy, try a random one
        httpd = socketserver.TCPServer(("", 0), handler)
        port = httpd.server_address[1]
    
    print(f"Starting DP visualizer web server on port {port}")
...
if __name__ == "__main__":
    print("Starting Longest Common Subsequence Visualizer...")
    # Try to handle the case when running this file directly
    try:
        start_server()
    except ImportError:
        # If the relative import fails, try to adjust using a direct import
        try:
            # Try using a direct import of lcs_algorithms
            import sys
            import os
